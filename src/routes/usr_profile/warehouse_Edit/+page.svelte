...
<script lang="ts">
	...
	const hasMatchingImage = async (itemName) => {
		...
		const matchingImages = matchingFolder.images.map((image) => {
			const imageName = image.name.toLowerCase()
			return `https://vyzeudiywhlxdzpnfehs.supabase.co/storage/v1/object/public/Gallery/items/${matchingFolder.name}/${imageName}`
		})

		return matchingImages.length > 0 ? matchingImages : null
	}
	...
	async function editItemHandling(event) {
		...
		const matchingImages = await hasMatchingImage(selectedProductName)
...
<section>
	...
	<select bind:value={warehouseHub}>
...
{:else if warehouseHub === 'Gallery'}
	...
	<form method="post" action="?/submit" use:enhance enctype="multipart/form-data">
		...
		<!-- Select Product -->
		<label for="product_id">Product</label>
		<select name="product_id" required aria-label="Product Selection" on:change={updateProductName}>
			<option value="">-- select a product --</option>
			{#each productData as product}
...
{:else if warehouseHub === 'Edit'}
	...
	>
		<label for="product_id">Product</label>
		<select name="product_id" required aria-label="Product Selection" on:change={editItemHandling}>
			<option value="">-- select a product --</option>
			{#each productData as product}
				<option
		...
		{#if matchingInventory.length > 0}
			...
			{#each matchingInventory as inventory}
				<div class="inventory-row">
					...
					<label for="size_{inventory.size_id}">Size</label>
					<input
						type="text"
						id="size_{inventory.size_id}"
						name="sizes[]"
						value={inventory.size_id}
						readonly
						aria-label="Size"
					/>

					<label for="quantity_{inventory.size_id}">Quantity</label>
					<input
						type="number"
					...
					<label for="reorder_threshold_{inventory.size_id}">Reorder Threshold</label>
					<input
						type="number"
